 ...
 * If the center sub display contains "INV" it overwrites the main display's current contents with the radian equivalent
 * of the main display's current content's. If not it overwrites the main display's current contents with the degree
 * equivalent of the main display's current content's.
 * @author Gary Buyn and Gavin Moore
 */
public class Deg extends Functionality
{
    public Deg(CalculatorWindow window)
    {
	super(window);
    }
	...
	// Checks for the inverse function
        if(currentSubDisplayC.compareTo("INV") == 0)
            {
                outputAnswer(mWindow, doubleToStr(Math.toRadians(temp)));
                currentSubDisplayC = "";
                mWindow.setSubDisplay(1, currentSubDisplayC);
            }
        else
            {
                outputAnswer(mWindow, doubleToStr(Math.toDegrees(temp)));
            }
